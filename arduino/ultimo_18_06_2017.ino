...
#define MAX_ANG -5.8
#define VEL_MAX   500
#define GRADO_MEDIO_H 96
#define GRADO_MEDIO_V 32
...
String instruccion, movimientoSH, movimientoSV;
int gananciaH = 0;
int gananciaV = 0;
int velocidadServos;
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (comandoRecibido.length() == 5) {
      instruccion = comandoRecibido.substring(0, 2);
      movimientoSH = comandoRecibido.substring(2, 3);
      movimientoSV = comandoRecibido.substring(3, 4);
      velocidadServos = stringToNumber(comandoRecibido.substring(4, 5));//es un integer
      
      Serial.print(instruccion);
      Serial.print(" ");
      Serial.print(movimientoSH);
      Serial.print(" ");
      Serial.println(movimientoSV);
      ...
      //        moverServoVertical(movimientoSV);
      //        comandoRecibido = "";
      //      }
      //      if (!(instruccion.compareTo("SH"))) {
      //        moverServoHorizontal(movimientoSH);
      //        comandoRecibido = "";
      //      }
      ...
      if (!(instruccion.compareTo("VE"))) {
        servoVector(movimientoSH, movimientoSV);
        comandoRecibido = "";
      }
      if (instruccion.compareTo("VE")) {
        comandoRecibido = "";
...
//}
void servoVector(String shAngulo, String svAngulo) {
  int GH;
  int GV;
  ...
  GH = GRADO_MEDIO_H + (gananciaH * velocidadServos);
  GV = GRADO_MEDIO_V + (gananciaV * velocidadServos);
    Serial.println(GH);
  Serial.println(GV);
  if (GH >= 180) {
    GH = 180;
  ...
  servoHorizontal.write(GH);
  servoVertical.write(GV);
}
